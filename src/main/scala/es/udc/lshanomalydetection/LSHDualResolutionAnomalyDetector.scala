...
trait LSHDualResolutionAnomalyDetectorParams extends Params
{
  final val smallBucketSize=new Param[Int](this, "desiredSmallSize", "Resolution of the smaller buckets. Only used if no keyLength, numTables and radius are provided.")
  final val largeBucketSize=new Param[Int](this, "desiredLargeSize", "Resolution of the smaller buckets. Only used if no keyLength, numTables and radius are provided.")
...
class LSHDualResolutionAnomalyDetectorModel(private val hashCountsSmall:scala.collection.Map[Hash,Int], private val hashCountsLarge:scala.collection.Map[Hash,Int], private val hasherSmall:Hasher, private val hasherLarge:Hasher, private val threshold:Int, private val radiusSmall:Double, private val radiusLarge:Double)
  ...
  with Serializable
{
  ...
  {
    //TODO
    val hashes=hasherSmall.getHashes(Vectors.dense(features.toArray), -1, radiusSmall)
    if (hashes.map({case (h,id) => hashCountsSmall.getOrElse(h, 0)}).sum<=threshold)
      return 1.0
    return 0.0
  }
  
  def getEstimatorSmall(features:Vector):Double=
  {
    val hashes=hasherSmall.getHashes(Vectors.dense(features.toArray), -1, radiusSmall)
    return -hashes.map({case (h,id) => hashCountsSmall.getOrElse(h, 0)}).sum
  }
  
  def getEstimatorLarge(features:Vector):Double=
  {
    val hashes=hasherLarge.getHashes(Vectors.dense(features.toArray), -1, radiusLarge)
    return -hashes.map({case (h,id) => hashCountsLarge.getOrElse(h, 0)}).sum
  }
  
  def getEstimator(features:Vector):Double=
  {
    return getEstimatorSmall(features)/getEstimatorLarge(features)
  }
...
class LSHDualResolutionAnomalyDetector(override val uid: String)
  extends LSHDualResolutionAnomalyDetectorParams
  with Logging
{
  ...
  def setSmallBucketSize(v:Int):this.type=set(smallBucketSize, v)
  setDefault(smallBucketSize, 5)
  def setLargeBucketSize(v:Int):this.type=set(largeBucketSize, v)
  setDefault(largeBucketSize, 100)
  ...
  def fit(rddData: RDD[LabeledPoint]): LSHDualResolutionAnomalyDetectorModel=
  {
    val dataRDD=rddData.zipWithIndex()
                        .map(_.swap)
                        .partitionBy(new HashPartitioner($(numPartitions)))
                        
    val small=fitForSize(dataRDD, $(smallBucketSize))
    val large=fitForSize(dataRDD, $(largeBucketSize))
    
    return new LSHDualResolutionAnomalyDetectorModel(small._1, large._1, small._2, large._2, small._3, small._4, large._4)
  }
  
  def fitForSize(dataRDD: RDD[(Long,LabeledPoint)], desiredSize:Int): (scala.collection.Map[Hash,Int], Hasher, Int, Double)=
  {
    ...
    return (hashedDataRDD.map({case (hash,neighbors) => (hash,neighbors.size)}).collectAsMap(),
          newHasher,
...
object LSHDualResolutionAnomalyDetector
{
...
Advanced LSH options:
  ...
  def evaluateModel(model:LSHDualResolutionAnomalyDetectorModel, testDataRDD:RDD[LabeledPoint]):Double=
  {
    //TODO
    ...
    val checkRDD = testDataRDD.map(
                                    {
                                      case lp =>
                                        ...
                                        (m.predict(f)>0,m.getEstimator(f),m.getEstimatorSmall(f),m.getEstimatorLarge(f),lp.label==ANOMALY_VALUE)
                                    })
                                    
    val confMat =  checkRDD.map(
                                  {
                                    case (pred,estimator,estimatorSmall,estimatorLarge,label) => 
      ...
      val metricsForEstimation = new BinaryClassificationMetrics(checkRDD.map({case (pred,estimator,estimatorSmall,estimatorLarge,label) => (estimator, if (label) 1.toDouble else 0.toDouble)}))
      val metricsForEstimationSmall = new BinaryClassificationMetrics(checkRDD.map({case (pred,estimator,estimatorSmall,estimatorLarge,label) => (estimatorSmall, if (label) 1.toDouble else 0.toDouble)}))
      val metricsForEstimationLarge = new BinaryClassificationMetrics(checkRDD.map({case (pred,estimator,estimatorSmall,estimatorLarge,label) => (estimatorLarge, if (label) 1.toDouble else 0.toDouble)}))
      val metricsForPrediction = new BinaryClassificationMetrics(checkRDD.map({case (pred,estimator,estimatorSmall,estimatorLarge,label) => (if (pred) 1.toDouble else 0.toDouble, if (label) 1.toDouble else 0.toDouble)}))
      // AUROC
      val auROCForEstimation = metricsForEstimation.areaUnderROC
      val auROCForEstimationSmall = metricsForEstimationSmall.areaUnderROC
      val auROCForEstimationLarge = metricsForEstimationLarge.areaUnderROC
      ...
      */
      ...
      println("\tArea under ROC (estimation)= " + auROCForEstimation)
      println("\tArea under ROC (estimation small)= " + auROCForEstimationSmall)
      println("\tArea under ROC (estimation large)= " + auROCForEstimationLarge)
  ...
  def main(args: Array[String])
  {
    ...
    val model=
      ...
      {
        new LSHDualResolutionAnomalyDetector()
      ...
      else
      {
        new LSHDualResolutionAnomalyDetector()
                        .setNumPartitions(options("num_partitions").asInstanceOf[Double].toInt)
                        //AUTO TUNING
                        .setSmallBucketSize(20)
                        .setLargeBucketSize(100)
